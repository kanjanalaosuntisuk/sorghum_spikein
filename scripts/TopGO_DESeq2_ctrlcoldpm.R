...
##### Effect of chilling stress on gene expression in PM
...
##### control_pm upregulated genes
### DESeq2 nospike normalization ---------------------
nospike_ctrl <- read.delim("output/DEGs_DESeq2_nospike_ctrlcoldpm_ctrl.txt", header = FALSE, sep = "")
...
nospike_ctrl_GOdata <- new("topGOdata", description = "DESeq2 nospike ctrl up", 
...
nospike_ctrl_allRes2[1,6] <- c("1e-30")
...
# save RData
saveRDS(nospike_ctrl_allRes4, file = "RData/TopGO_ctrlcoldpm_nospike.RData")


#####################################################
### DESeq2 nudelta normalization ---------------------
nudelta_ctrl <- read.delim("output/DEGs_DESeq2_nudelta_ctrlcoldpm_ctrl.txt", header = FALSE, sep = "")
...
nudelta_ctrl_allRes2[1,6] <- c("1e-30")
...
# save RData
saveRDS(nudelta_ctrl_allRes4, file = "RData/TopGO_ctrlcoldpm_nudelta.RData")


#####################################################
### DESeq2 spikesf normalization ---------------------
spikesf_ctrl <- read.delim("output/DEGs_DESeq2_spikesf_ctrlcoldpm_ctrl.txt", header = FALSE, sep = "")
...
spikesf_ctrl_allRes2 <- subset(spikesf_ctrl_allRes, 
                             as.numeric(spikesf_ctrl_allRes[,"weight01Fisher"])<0.001 | grepl("e-", spikesf_ctrl_allRes[,"weight01Fisher"]))
spikesf_ctrl_allRes2$genes <-vapply(spikesf_ctrl_allRes2$genes, paste, collapse = ",", character(1L))
spikesf_ctrl_allRes2[1,6] <- c("1e-30")
...
RUV_ctrl <- read.delim("output/DEGs_DESeq2_RUV_ctrlcoldpm_ctrl.txt", header = FALSE, sep = "")
...
RUV_ctrl_allRes2 <- subset(RUV_ctrl_allRes, 
                             as.numeric(RUV_ctrl_allRes[,"weight01Fisher"])<0.001 | grepl("e-", RUV_ctrl_allRes[,"weight01Fisher"]))
RUV_ctrl_allRes2$genes <-vapply(RUV_ctrl_allRes2$genes, paste, collapse = ",", character(1L))
RUV_ctrl_allRes2[1,6] <- c("1e-30")
RUV_ctrl_allRes2 %>%
  mutate(adj_pval = as.numeric(weight01Fisher)) %>%
  dplyr::rename(ID = GO.ID, Genes = genes) %>%
...
# save plot
png("TopGO_pheatmap_DESeq2_ctrlcoldpm_ctrl.png", width = 5, height = 7, units = "in", res = 300)
p
dev.off()

##### compare only median of ratio vs nudelta
# load RData
nospike_ctrl_allRes4 <- readRDS(file = "RData/TopGO_ctrlcoldpm_nospike.RData")
nudelta_ctrl_allRes4 <- readRDS(file = "RData/TopGO_ctrlcoldpm_nudelta.RData")
...
p <- pheatmap(as.matrix(ctrl_up_GO_2norm2), cluster_row = FALSE, cluster_cols = FALSE,
              display_numbers = as.matrix(ctrl_up_GO_genes_2norm2), fontsize = 10,
              annotation_legend = TRUE)
# save plot
png("TopGO_pheatmap_DESeq2_ctrlcoldpm_ctrl_2norm.png", width = 7, height = 8, units = "in", res = 300)
p
dev.off()
...
####################################################################
####################################################################
##### chilling_pm upregulated genes
### DESeq2 nospike normalization ---------------------
nospike_cold <- read.delim("output/DEGs_DESeq2_nospike_ctrlcoldpm_cold.txt", header = FALSE, sep = "")
...
# save RData
saveRDS(nospike_cold_allRes4, file = "RData/TopGO_ctrlcoldpm_nospike_cold.RData")


####################################################################
### DESeq2 nudelta normalization ---------------------
nudelta_cold <- read.delim("output/DEGs_DESeq2_nudelta_ctrlcoldpm_cold.txt", header = FALSE, sep = "")
...
# save RData
saveRDS(nudelta_cold_allRes4, file = "RData/TopGO_ctrlcoldpm_nudelta_cold.RData")


####################################################################
### DESeq2 spikesf normalization ---------------------
spikesf_cold <- read.delim("output/DEGs_DESeq2_spikesf_ctrlcoldpm_cold.txt", header = FALSE, sep = "")
...
spikesf_cold_allRes2 <- subset(spikesf_cold_allRes, 
                             as.numeric(spikesf_cold_allRes[,"weight01Fisher"])<0.001 | grepl("e-", spikesf_cold_allRes[,"weight01Fisher"]))
...
####################################################################
### DESeq2 RUV normalization ---------------------
RUV_cold <- read.delim("output/DEGs_DESeq2_RUV_ctrlcoldpm_cold.txt", header = FALSE, sep = "")
...
RUV_cold_allRes2 <- subset(RUV_cold_allRes, 
                             as.numeric(RUV_cold_allRes[,"weight01Fisher"])<0.001 | grepl("e-", RUV_cold_allRes[,"weight01Fisher"]))
...
p <- pheatmap(as.matrix(cold_up_GO), cluster_row = FALSE, cluster_cols = FALSE,
              display_numbers = as.matrix(cold_up_GO_genes), fontsize = 10,
              annotation_legend = TRUE)

# save plot
png("TopGO_pheatmap_DESeq2_ctrlcoldpm_cold.png", width = 6, height = 8, units = "in", res = 300)
p
dev.off()

##### compare only median of ratio vs NuDelta ----
# load RData
nospike_cold_allRes4 <- readRDS(file = "RData/TopGO_ctrlcoldpm_nospike_cold.RData")
nudelta_cold_allRes4 <- readRDS(file = "RData/TopGO_ctrlcoldpm_nudelta_cold.RData")
...
p <- pheatmap(as.matrix(cold_up_GO_2norm2), cluster_row = FALSE, cluster_cols = FALSE,
              display_numbers = as.matrix(cold_up_GO_genes_2norm2), fontsize = 10,
              annotation_legend = TRUE)

# save plot
png("TopGO_pheatmap_DESeq2_ctrlcoldpm_cold_2norm.png", width = 5, height = 12, units = "in", res = 300)
p
dev.off()
